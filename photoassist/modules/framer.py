...
class Framer(BaseModule):
	"""Формирует финальный кадр.

	Для класса 'apple' маска не используется; для остальных применяется сглаженная
	маска как альфа для умножения на изображение.
	"""
	...
	def _fork(self, input_data: Dict) -> Dict:
		"""Ветвление логики по классу объекта."""
	...
	def _process_other(self, input_data: Dict) -> Dict:
		"""Сглаживает и нормализует маску, умножает её на изображение как альфу."""
	...
	def _apply_transform(self, input_data):
		"""Возвращает финальное изображение после формирования кадра."""
